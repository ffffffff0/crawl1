...
我之前写过58同城的爬虫和这个类似， 但是写这个的时候bug明显更多，最坑的是美团，美团的数据加载使用的是 ajax(异步加载JavaScript和XML)，用selenium虽然可以,但是效率低。抓包分析效率高，但是很难模仿请求。所以要了解网站的一些常见的反爬措施和网页的结构。 赶集网这种网站页面结构很适合爬取， 但是它对同一ip的访问频率会有限制。这个程序的效率还可以优化，*例如 使用 lxml 来解析网页， 速度会快大概10倍的样子， 还可以利用 异步非阻塞 优化， *虽然我不会。 我会继续学习的。**
